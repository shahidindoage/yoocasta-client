...
import { ChevronDown, ChevronUp, MapPin, Currency } from 'lucide-react';
...
const MyJobInvitations = () => {
  ...
  const [draftFilters, setDraftFilters] = useState<Record<string, any>>({ sort: 'latest' });
  const [appliedFilters, setAppliedFilters] = useState<Record<string, any>>({ sort: 'latest' });
  ...
  const fetchInvitations = async (filters: Record<string, any>) => {
    setLoading(true);
    try {
      const res = await getMyInvitations(filters);
  ...
  useEffect(() => {
    fetchInvitations(appliedFilters);
  }, [appliedFilters]);
  ...
  return (
    <div style={{ background: '#ffffffff', minHeight: '100vh', fontFamily: 'system-ui, sans-serif', margin: 0 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
                <button
                  ...
                  style={{ background: '#3835A4', color: '#fff', border: 'none', borderRadius: '8px', padding: '12px 24px', fontWeight: 'bold', cursor: 'pointer', fontSize: '13px', transition: 'background 0.2s' }}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 40px 0', flexWrap: 'wrap', gap: '12px' }}>
        <h2 style={{ margin: 0, fontSize: '20px', fontWeight: 900, letterSpacing: '-0.02em', color: '#000000ff' }}>
          {jobs.length > 0 && <span style={{ color: '#3835a4', marginRight: '3px' }}>{jobs.length}</span>} Job Invitations Received
        </h2>
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          <select
            value={appliedFilters.sort || 'latest'}
            onChange={e => handleSortChange(e.target.value)}
            style={{ background: '#fff', color: '#000000ff', border: '1px solid #f5d0e3', padding: '8px 12px', borderRadius: '8px', fontSize: '12px', fontWeight: 'bold' }}
          >
      ...
      <div style={{ padding: '40px' }}>
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-10">
            {jobs.length === 0 ? (
              <p style={{ color: '#C6007E' }}>No job invitations found matching your criteria.</p>
            ) : (
              jobs.map((job: any) => {
                  ...
                  >
                    ...
                    <div className="p-8 flex flex-col flex-grow justify-between relative bg-white">
                      ...
                      <div className="pt-6 mt-6 border-t border-neutral-100 flex flex-col gap-4">
                        {job.castingCity && (
                          <div className="flex items-start gap-3.5 group/meta">
                            <div className="h-9 w-9 rounded-xl bg-neutral-50 border border-neutral-200/80 flex items-center justify-center shrink-0 transition-colors group-hover/meta:bg-[#C6007E]/5 group-hover/meta:border-[#C6007E]/20">
                              <MapPin className="h-4 w-4 text-[#C6007E]" />
                            </div>
                        ...
                        {firstRole && (formatBudget(firstRole) || firstRole.paymentType) && (
                          <div className="flex items-start gap-3.5 group/meta">
                            <div className="h-9 w-9 rounded-xl bg-[#3835A4]/5 border border-[#3835A4]/10 flex items-center justify-center shrink-0 transition-colors group-hover/meta:bg-[#3835A4]/10">
                              <Currency className="h-4 w-4 text-[#3835A4]" />
                            </div>
                            <div className="space-y-0.5">
                      ...
                      <div className="pt-6 mt-6 border-t border-dashed border-neutral-200 flex items-center justify-between">
                        <div>
                          {isExpired ? (
                            <span className="inline-flex items-center text-[9px] font-mono font-black bg-red-50 text-red-600 border border-red-200/60 px-2.5 py-1 rounded-lg tracking-wider">EXPIRED</span>
                          ) : (
                            <span className="inline-flex items-center gap-1 text-[9px] font-mono font-black bg-[#C6007E]/10 text-[#C6007E] border border-[#C6007E]/20 px-2.5 py-1 rounded-lg tracking-wider">
                        ...
                        {isExpired && (
                          <span className="text-[10px] font-mono font-black uppercase text-neutral-500">Application Closed</span>
                        )}
                      </div>
                    </div>
                  </div>
...
export default MyJobInvitations;