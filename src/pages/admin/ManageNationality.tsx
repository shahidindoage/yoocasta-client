...
const ManageNationality = () => {
  ...
  const handleEdit = (n: { id: string; name: string }) => {
    setEditId(n.id);
    setName(n.name);
    setActiveTab('add');
    setError('');
    ...
    <div>
      <div className="flex items-center gap-4 mb-6 border-b border-stone-200">
        <button onClick={() => handleTabChange('list')}
          className={`pb-3 text-sm font-bold cursor-pointer transition-colors ${activeTab === 'list' ? 'text-[#3835A4] border-b-2 border-[#3835A4]' : 'text-stone-400 hover:text-stone-600'}`}>
          Nationality List
        </button>
      ...
      {activeTab === 'list' ? (
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr><td colSpan={3} className="text-center py-12 text-stone-400 text-sm font-bold border-t border-stone-100">No nationalities found</td></tr>
                  ) : (
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-end gap-1 mt-6">
              ...
              {(() => {
                ...
                const t = totalPages, c = page;
                if (t <= 7) { for (let i = 1; i <= t; i++) pages.push(i); }
                else {
                  let s = c - 2, e = c + 2;
                  if (s < 1) { e += 1 - s; s = 1; }
                  if (e > t) { s -= e - t; e = t; }
                  s = Math.max(1, s); e = Math.min(t, e);
                  if (s > 1) { pages.push(1); if (s > 2) pages.push('...'); }
                  for (let i = s; i <= e; i++) pages.push(i);
                  if (e < t) { if (e < t - 1) pages.push('...'); pages.push(t); }
                }
                return pages.map((p, idx) =>
                  p === '...' ? <span key={`e-${idx}`} className="px-1 text-xs font-bold text-stone-400">...</span>
        ...
        <div>
          <h2 className="text-2xl font-black text-[#3835A4] mb-6">{editId ? 'Edit Nationality' : 'Add New Nationality'}</h2>
          ...
          <form onSubmit={handleSubmit} className="bg-white border border-stone-200 p-6 max-w-xl space-y-5">
            ...
            <div className="flex justify-end pt-4">
              ...
              <button type="submit" disabled={submitting}
                ...
                {submitting ? 'Saving...' : (editId ? 'Update Nationality' : 'Add Nationality')}
              </button>
            </div>
          </form>
        </div>
