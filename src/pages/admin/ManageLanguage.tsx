...
const ManageLanguage = () => {
  ...
  const openEdit = (l: { id: string; name: string }) => {
    setEditId(l.id); setName(l.name); setError(''); setSuccess(''); setModalOpen(true);
  };
      ...
      <div className="bg-white border border-stone-200"><div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            : languages.length === 0 ? <tr><td colSpan={3} className="text-center py-12 text-stone-400 text-sm font-bold border-t border-stone-100">No languages found</td></tr>
            : languages.map((l, idx) => (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={(e) => { if (e.target === e.currentTarget) closeModal(); }}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-5">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-black text-[#3835A4]">{editId ? 'Edit Language' : 'Add New Language'}</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)}
                  className="w-full bg-transparent border-b-2 border-stone-200 py-3 text-sm outline-none focus:border-[#3835A4]" placeholder="Enter language name" autoFocus />
              </div>
              <div className="flex justify-end gap-3">
                ...
                <button type="submit" disabled={submitting}
                  ...
                  {submitting ? 'Saving...' : (editId ? 'Update Language' : 'Add Language')}</button>
              </div>
            </form>
          </div>
        </div>
