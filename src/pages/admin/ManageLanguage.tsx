...
import { getLanguages, createLanguage, deleteLanguage, updateLanguage } from '../../api/admin.api';
...
const ManageLanguage = () => {
  ...
  const fetch = async (p: number) => {
    setLoading(true);
    try {
      const res = await getLanguages(p, 20);
  ...
  const handleEdit = (l: { id: string; name: string }) => {
    setEditId(l.id);
    setName(l.name);
    setActiveTab('add');
    setError('');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editId) {
        const res = await updateLanguage(editId, name.trim());
        setLanguages((prev) => prev.map((l) => l.id === editId ? res.data.data : l));
        setSuccess('Language updated successfully');
      } else {
        const res = await createLanguage(name.trim());
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-6 border-b border-stone-200">
        <button
          ...
          className={`pb-3 text-sm font-bold cursor-pointer transition-colors ${
            activeTab === 'list' ? 'text-[#3835A4] border-b-2 border-[#3835A4]' : 'text-stone-400 hover:text-stone-600'
          }`}
        >
          Language List
        </button>
        <button
          ...
          className={`pb-3 text-sm font-bold cursor-pointer transition-colors ${
            activeTab === 'add' ? 'text-[#3835A4] border-b-2 border-[#3835A4]' : 'text-stone-400 hover:text-stone-600'
          }`}
        >
      ...
      {activeTab === 'list' ? (
        <>
          <div className="mb-6">
            <h2 className="text-2xl font-black text-[#3835A4]">Language List</h2>
            <p className="text-xs text-stone-400">{total} total languages</p>
          </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-stone-50 border-b border-stone-200">
                    {['Sl No', 'Language Name', 'Action'].map((h) => (
                ...
                <tbody>
                  {loading ? (
                    <tr><td colSpan={3} className="text-center py-12 text-stone-400 text-sm font-bold border-t border-stone-100">Loading...</td></tr>
                  ) : languages.length === 0 ? (
                    <tr><td colSpan={3} className="text-center py-12 text-stone-400 text-sm font-bold border-t border-stone-100">No languages found</td></tr>
                  ) : (
                    languages.map((l, idx) => (
                      <tr key={l.id} className="border-b border-stone-100 hover:bg-stone-50">
                        ...
                        <td className="px-4 py-3 font-medium border-r border-stone-100">{l.name}</td>
                  ...
                  p === '...' ? (
                    ...
                    <button key={p} onClick={() => setPage(p as number)}
                      className={`w-8 h-8 text-xs font-bold cursor-pointer border border-stone-200 ${
                        page === p ? 'bg-[#3835A4] text-white border-[#3835A4]' : 'text-stone-500 hover:bg-stone-100'
                      }`}>{p}</button>
                  ),
                );
              })()}
        ...
        <div>
          <h2 className="text-2xl font-black text-[#3835A4] mb-6">{editId ? 'Edit Language' : 'Add New Language'}</h2>
          ...
          <form onSubmit={handleSubmit} className="bg-white border border-stone-200 p-6 max-w-xl space-y-5">
            <div>
              <label className="block text-[10px] font-bold text-stone-500 uppercase mb-2">Language Name *</label>
              <input
            ...
            <div className="flex justify-end pt-4">
              ...
              >
                {submitting ? 'Saving...' : (editId ? 'Update Language' : 'Add Language')}
              </button>
            </div>
          </form>
        </div>
