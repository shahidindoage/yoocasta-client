...
import { getEthnicities, createEthnicity, deleteEthnicity, updateEthnicity } from '../../api/admin.api';
import { Trash2, Pencil } from 'lucide-react';

const ManageEthnicity = () => {
  ...
  const fetch = async (p: number) => {
    setLoading(true);
    try {
      const res = await getEthnicities(p, 20);
      const d = res.data.data;
      setItems(d.ethnicities || []);
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Delete this ethnicity?')) return;
    try { await deleteEthnicity(id); setItems((prev) => prev.filter((i) => i.id !== id)); setTotal((prev) => prev - 1); }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editId) {
        const res = await updateEthnicity(editId, name.trim());
        setItems((prev) => prev.map((i) => i.id === editId ? res.data.data : i));
        setSuccess('Ethnicity updated successfully');
      } else {
        const res = await createEthnicity(name.trim());
        setItems((prev) => [...prev, res.data.data]);
        setTotal((prev) => prev + 1);
        setSuccess('Ethnicity added successfully');
      }
    ...
    <div>
      <div className="flex items-center gap-4 mb-6 border-b border-stone-200">
        <button onClick={() => handleTabChange('list')}
          className={`pb-3 text-sm font-bold cursor-pointer transition-colors ${activeTab === 'list' ? 'text-[#3835A4] border-b-2 border-[#3835A4]' : 'text-stone-400 hover:text-stone-600'}`}>Ethnicity List</button>
        <button onClick={() => handleTabChange('add')}
          ...
          {editId ? 'Edit Ethnicity' : 'Add Ethnicity'}</button>
      </div>

      {activeTab === 'list' ? (
        <>
          <div className="mb-6">
            <h2 className="text-2xl font-black text-[#3835A4]">Ethnicity List</h2>
            <p className="text-xs text-stone-400">{total} total ethnicities</p>
          </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-stone-50 border-b border-stone-200">
                    {['Sl No', 'Ethnicity Name', 'Action'].map((h) => (
                ...
                <tbody>
                  ...
                  : items.length === 0 ? <tr><td colSpan={3} className="text-center py-12 text-stone-400 text-sm font-bold border-t border-stone-100">No ethnicities found</td></tr>
                  : items.map((item, idx) => (
        ...
        <div>
          <h2 className="text-2xl font-black text-[#3835A4] mb-6">{editId ? 'Edit Ethnicity' : 'Add New Ethnicity'}</h2>
          ...
          <form onSubmit={handleSubmit} className="bg-white border border-stone-200 p-6 max-w-xl space-y-5">
            <div>
              <label className="block text-[10px] font-bold text-stone-500 uppercase mb-2">Ethnicity Name *</label>
              <input type="text" value={name} onChange={(e) => setName(e.target.value)}
                className="w-full bg-transparent border-b-2 border-stone-200 py-3 text-sm outline-none focus:border-[#3835A4]" placeholder="Enter ethnicity name" />
            </div>
            <div className="flex justify-end pt-4">
              ...
              <button type="submit" disabled={submitting}
                ...
                {submitting ? 'Saving...' : (editId ? 'Update Ethnicity' : 'Add Ethnicity')}</button>
            </div>
          </form>
        </div>
      )}
...
export default ManageEthnicity;
