...
const ManageEthnicity = () => {
  ...
  const openEdit = (item: { id: string; name: string }) => {
    setEditId(item.id); setName(item.name); setError(''); setSuccess(''); setModalOpen(true);
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={openAdd}
          ...
          <Plus className="w-4 h-4" /> Add Ethnicity
        </button>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={(e) => { if (e.target === e.currentTarget) closeModal(); }}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-5">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-black text-[#3835A4]">{editId ? 'Edit Ethnicity' : 'Add New Ethnicity'}</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)}
                  className="w-full bg-transparent border-b-2 border-stone-200 py-3 text-sm outline-none focus:border-[#3835A4]" placeholder="Enter ethnicity name" autoFocus />
              </div>
