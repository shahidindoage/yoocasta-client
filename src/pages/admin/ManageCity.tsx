...
const ManageCity = () => {
  ...
  const handleEdit = (item: { id: string; name: string; countryId: string | null }) => {
    setEditId(item.id); setName(item.name); setCountryId(item.countryId || ''); setActiveTab('add'); setError(''); setSuccess('');
  };
  ...
  const handleTabChange = (tab: 'list' | 'add') => {
    if (tab === 'add' && activeTab !== 'add') { setName(''); setCountryId(''); setEditId(null); setError(''); setSuccess(''); }
    setActiveTab(tab);
  };
    ...
    <div>
      <div className="flex items-center gap-4 mb-6 border-b border-stone-200">
        <button onClick={() => handleTabChange('list')}
          className={`pb-3 text-sm font-bold cursor-pointer transition-colors ${activeTab === 'list' ? 'text-[#3835A4] border-b-2 border-[#3835A4]' : 'text-stone-400 hover:text-stone-600'}`}>City List</button> 
        <button onClick={() => handleTabChange('add')}
          ...
          {editId ? 'Edit City' : 'Add City'}</button>
      </div>
      ...
      {activeTab === 'list' ? (
        ...
        <div>
          <h2 className="text-2xl font-black text-[#3835A4] mb-6">{editId ? 'Edit City' : 'Add New City'}</h2>
          ...
          <form onSubmit={handleSubmit} className="bg-white border border-stone-200 p-6 max-w-xl space-y-5">
            ...
            <div>
              ...
              <input type="text" value={name} onChange={(e) => setName(e.target.value)}
                className="w-full bg-transparent border-b-2 border-stone-200 py-3 text-sm outline-none focus:border-[#3835A4]" placeholder="Enter city name" />
            </div>
