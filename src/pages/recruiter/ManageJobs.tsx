 ...
 if (loading) {
  return (
   <div className="min-h-screen flex items-center justify-center bg-neutral-50 text-[#3835A4]">
    <div className="animate-pulse text-xs font-black ">Loading Ledger...</div>
   </div>
  );
 }
  ...
  <div className="min-h-screen bg-neutral-50 text-[#3835A4] font-sans p-4 md:p-8 lg:p-12">
   <div className="max-w-6xl mx-auto space-y-8">
    ...
    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-[#3835A4]/10">
     <div>
      {/* <span className="text-[9px] font-black text-[#C6007E] block mb-1">Recruiter Console</span> */}
      <h1 className="text-3xl md:text-4xl font-black">Manage Jobs</h1>
     </div>
     <Link to="/dashboard/recruiter/post-job" className="bg-[#C6007E] text-white px-6 py-3 rounded-xl font-black  text-xs hover:bg-[#a10065] transition-colors whitespace-nowrap">
      + Post New Job
     </Link>
    </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
     <div className="bg-white border-2 border-[#3835A4]/10 rounded-2xl p-6 shadow-sm flex items-center justify-between">
      <div>
       <h3 className="text-[10px] font-extrabold text-[#3835A4]/50 ">Total Jobs Posted</h3>
     ...
     <div className="bg-white border-2 border-[#3835A4]/10 rounded-2xl p-6 shadow-sm flex items-center justify-between">
      <div>
       <h3 className="text-[10px] font-extrabold text-[#3835A4]/50 ">Total Applications</h3>
    ...
    <div className="flex flex-col sm:flex-row gap-4 bg-white p-4 rounded-xl border border-[#3835A4]/10 shadow-sm">
     <div className="flex-1 space-y-1">
      <label className="text-[9px] font-bold  text-[#3835A4]/50">Filter By Status</label>
      <select 
       value={filter} 
     ...
     <div className="flex-1 space-y-1">
      <label className="text-[9px] font-bold  text-[#3835A4]/50">Sort By</label>
      <select 
       value={sort} 
     ...
     <table className="w-full text-left border-collapse min-w-[800px]">
      <thead>
       <tr className="bg-[#3835A4]/5 border-b border-[#3835A4]/10">
        <th className="p-4 text-[10px] font-black  text-[#3835A4]/60">Title</th>
        <th className="p-4 text-[10px] font-black  text-[#3835A4]/60 text-center">Applications</th>
        <th className="p-4 text-[10px] font-black  text-[#3835A4]/60 text-center">Shortlisted</th>
        <th className="p-4 text-[10px] font-black  text-[#3835A4]/60 text-center">Selected</th>
        <th className="p-4 text-[10px] font-black  text-[#3835A4]/60 text-center">Views</th>
        <th className="p-4 text-[10px] font-black  text-[#3835A4]/60">Created</th>
        <th className="p-4 text-[10px] font-black  text-[#3835A4]/60">Expired</th>
        <th className="p-4 text-[10px] font-black  text-[#3835A4]/60">Status</th>
        <th className="p-4 text-[10px] font-black  text-[#3835A4]/60 text-right">Action</th>
       </tr>
      </thead>
        ...
        processedJobs.map((job) => {
         ...
         return (
          <tr key={job.id} className="hover:bg-[#3835A4]/[0.02] transition-colors">
           ...
           <td className="p-4">
            {isExpired ? (
             <span className="bg-red-50 text-red-600 border border-red-200 px-2 py-1 rounded text-[10px] font-bold ">Expired</span>
            ) : job.status === 'APPROVED' ? (
             <span className="bg-green-50 text-green-600 border border-green-200 px-2 py-1 rounded text-[10px] font-bold ">Active</span>
            ) : job.status === 'PENDING' ? (
             <span className="bg-orange-50 text-orange-600 border border-orange-200 px-2 py-1 rounded text-[10px] font-bold ">Inactive</span>
            ) : (
