    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-8 py-10 min-h-screen ">
      <div className="space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-black tracking-tight text-[#3835A4] uppercase">Cast Bags</h1>
            <p className="text-sm text-stone-500 font-medium mt-1">Manage your talent collections</p>
          </div>
          <button
            onClick={() => setShowCreate(true)}
            className="bg-[#C6007E] text-white px-6 py-3 rounded-xl font-black uppercase tracking-widest text-xs hover:bg-[#a10065] transition-all"
          >
            + Create Cast Bag
          </button>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 space-y-4">
            ...
            <button onClick={() => setShowCreate(true)} className="text-[10px] font-black uppercase tracking-widest bg-[#3835A4] text-white px-6 py-3 rounded-xl hover:bg-[#2a2899] transition-all">
          ...
          <div className="bg-white rounded-2xl border border-stone-100 overflow-hidden shadow-sm">
            <table className="w-full text-left">
              <thead>
                <tr className="bg-stone-50 text-[9px] font-black uppercase tracking-widest text-stone-500">
                    ...
                    <td className="px-5 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          className="text-[9px] font-black uppercase tracking-wider bg-[#3835A4] text-white px-3 py-2 rounded-lg hover:bg-[#2a2899] transition-all"
                        >
                          📧 Email Link
                        </button>
                        <button
                          onClick={() => handleDelete(bag.id)}
                          className="text-[9px] font-black uppercase tracking-wider bg-red-500 text-white px-3 py-2 rounded-lg hover:bg-red-600 transition-all"
                        >
                          Delete
                        </button>
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={e => { if (e.target === e.currentTarget) setShowCreate(false); }}>
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 space-y-5" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-extrabold tracking-widest text-stone-400 uppercase">Name *</label>
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button onClick={() => setShowCreate(false)} className="text-xs font-bold text-stone-500 hover:text-stone-800 uppercase tracking-wider">Cancel</button>
              <button onClick={handleCreate} disabled={!newName.trim()} className="bg-[#C6007E] disabled:opacity-40 text-white px-6 py-2.5 rounded-xl font-black uppercase tracking-widest text-xs hover:bg-[#a10065] transition-all">Save</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showShare && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={e => { if (e.target === e.currentTarget) setShowShare(null); }}>
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6 space-y-5" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-extrabold tracking-widest text-stone-400 uppercase">Email ID(s) *</label>
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-extrabold tracking-widest text-stone-400 uppercase">Link Expires In (days)</label>
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button onClick={() => setShowShare(null)} className="text-xs font-bold text-stone-500 hover:text-stone-800 uppercase tracking-wider">Cancel</button>
              <button onClick={handleShare} disabled={!shareEmails.trim() || sharing} className="bg-[#C6007E] disabled:opacity-40 text-white px-6 py-2.5 rounded-xl font-black uppercase tracking-widest text-xs hover:bg-[#a10065] transition-all flex items-center gap-2">
                {sharing ? <>
